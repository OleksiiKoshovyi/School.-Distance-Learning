...
namespace School._Distance_Learning.ViewModels.TimetableForTeachers
{
    public class CsvResult : FileResult
    {
        ...
        public CsvResult(IndexViewModel timetableData) : base("text/csv")
        {
            ...
            FileDownloadName = $"{timetableData.teacher.PartName}:{timetableData.date}.csv";
        }
        ...
        public async override Task ExecuteResultAsync(ActionContext context)
        {
            ...
            using (var streamWriter = new StreamWriter(response.Body, System.Text.Encoding.UTF8))
            {
                ...
                await streamWriter.WriteLineAsync(
                  $";{_timetableData.teacher.SurName};{_timetableData.teacher.FirstName};{_timetableData.teacher.Patronymic}"
                );
                    ...
                    await streamWriter.WriteLineAsync(
                        ...
                        _timetableData.timetable[i].Select(
                            t => string.Join(" / ", t.Select(
                                l => $"{l.TeacherSubjectGroup.Group.Grade.GradeName}:{l.TeacherSubjectGroup.TeacherSubject.Subject.SubjectName}"))))
                    );
